...
const seedDir = path.join(__dirname, '..', 'public', 'seed-images');

// Zkopíruje real-* fotky z public/seed-images/ (v gitu) do public/uploads/
// (které může být persistent-disk mount na Renderu — mount skryje git soubory).
function copySeedFiles() {
  try {
    if (!fs.existsSync(uploadsDir)) fs.mkdirSync(uploadsDir, { recursive: true });
    if (!fs.existsSync(seedDir)) {
      console.warn('[ensure-seed] seed-images adresář neexistuje');
      return 0;
    }
    const seedFiles = fs.readdirSync(seedDir).filter(f => f.startsWith('real-') || f.startsWith('thumb-real-'));
    let copied = 0;
    ...
    if (copied > 0) console.log(`[ensure-seed] zkopírováno ${copied} seed fotek do uploads/`);
    return copied;
  } catch (err) {
    console.error('[ensure-seed] copy chyba:', err.message);
    return 0;
  }
}
...
function ensureSeed() {
  try {
    // Krok 1: nakopíruj fotky z gitu (public/seed-images/) do uploads/
    // Nezbytné na Renderu — persistent disk mount může skrýt git soubory v uploads/
    copySeedFiles();
